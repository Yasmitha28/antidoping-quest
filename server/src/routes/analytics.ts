...
export default function analyticsRoutes(prisma: PrismaClient) {
  const router = Router();
  ...
  router.get('/dashboard', authMiddleware, async (req, res) => {
    ...
    const topUsers = await withFallback(
      () => prisma.user.findMany({ take: 5, orderBy: { xp: 'desc' }, select: { id: true, name: true, xp: true, level: true } }),
      () => fallbackStore.listTopUsers()
    );

...
export default function challengeRoutes(prisma: PrismaClient) {
  const router = Router();

  router.get('/', async (_req, res) => {
    const challenges = await withFallback(
      () => prisma.challenge.findMany({ orderBy: { createdAt: 'asc' } }),
      () => fallbackStore.listChallenges()
    );
    res.json({ challenges });
  });
  ...
  router.post('/:id/complete', authMiddleware, async (req, res) => {
    ...
    const challenge = await withFallback(
      () => prisma.challenge.findUnique({ where: { id: challengeId } }),
      () => fallbackStore.listChallenges().then((items) => items.find((item) => item.id === challengeId) || null)
    );

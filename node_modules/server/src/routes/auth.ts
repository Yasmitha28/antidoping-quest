...
export default function authRoutes(prisma: PrismaClient) {
  ...
  router.post('/register', async (req, res) => {
    ...
    const user = await withFallback(
      async () => {
        ...
        if (existing) {
          return { error: true, status: 409, message: 'Email already exists', user: null as any };
        }
  ...
  router.get('/me', authMiddleware, async (req, res) => {
    ...
    const profile = await withFallback(
      async () => prisma.user.findUnique({ where: { id: user.id } }),
      async () => fallbackStore.findUserById(user.id)
    );
    if (!profile) {
      return res.status(404).json({ error: 'User not found' });
    }
    const certificates = await withFallback(
      () => prisma.certificate.findMany({ where: { userId: profile.id }, orderBy: { issuedAt: 'desc' } }),
      () => fallbackStore.listCertificates(profile.id)
    );
    return res.json({ user: { id: profile.id, name: profile.name, email: profile.email, role: profile.role, xp: profile.xp, level: profile.level, streak: profile.streak, badges: profile.badges, certificates } });
  });
